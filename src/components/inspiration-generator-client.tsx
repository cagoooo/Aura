...
import { Loader2, Sparkles, CheckCircle2, Shuffle } from 'lucide-react';
...
export default function InspirationGeneratorClient() {
  ...
  const handleRandomGenerate = useCallback((key: W1HKey) => {
    ...
    // Simulate async for potential future AI random generation
    setTimeout(() => {
  ...
  const handleGrammarRefinement = async () => {
    ...
    try {
      ...
      setW1hData((prev) => {
        const newState = { ...prev };
        for (const key of ALL_W1H_KEYS) {
          if (!newState[key].isLocked) { // Only update unlocked items, or all items? Let's update all for refinement.
  ...
  // Initial random generation for any empty fields (though constructor handles this now)
  useEffect(() => {
    let updated = false;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <p className="text-center text-lg text-foreground mb-8">
        點擊「隨機產生」來獲得靈感，或使用AI工具「潤飾語法」及「檢查一致性」來完善您的創意點子！
      </p>

      <div className="flex flex-col sm:flex-row gap-4 mb-8 justify-center">
        <Button onClick={handleRandomAll} disabled={isLoading.randomAll || isLoading.grammar || isLoading.consistency} className="bg-accent hover:bg-accent/90 text-accent-foreground flex-1 sm:flex-none rounded-lg shadow-md">
          {isLoading.randomAll ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Shuffle className="mr-2 h-5 w-5" />}
          全部隨機
        </Button>
        <Button onClick={handleGrammarRefinement} disabled={isLoading.grammar || isLoading.randomAll || isLoading.consistency} className="bg-primary hover:bg-primary/90 text-primary-foreground flex-1 sm:flex-none rounded-lg shadow-md">
          {isLoading.grammar ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Sparkles className="mr-2 h-5 w-5" />}
          潤飾語法 (AI)
        </Button>
        <Button onClick={handleConsistencyCheck} disabled={isLoading.consistency || isLoading.randomAll || isLoading.grammar} className="bg-primary hover:bg-primary/90 text-primary-foreground flex-1 sm:flex-none rounded-lg shadow-md">
              ...
              : (
                <ul>
                  {consistencyResult.suggestions.map((suggestion, index) => (
                    <li key={index} className="list-disc list-inside ml-2">{suggestion}</li>
                  ))}
                </ul>
              )
            }
